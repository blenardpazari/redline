...
def lookup(ip: str) -> tuple[str | None, float | None, float | None]:
    ...
    result = _try_ipinfo(ip) or _try_ipapi(ip) or (None, None, None)
    _cache[ip] = result
    return result


def _try_ipinfo(ip: str) -> tuple[str | None, float | None, float | None] | None:
    try:
        ...
        if "loc" in data and data.get("country"):
            ...
            return (data["country"], float(lat), float(lon))
    except Exception:
        pass
    return None


def _try_ipapi(ip: str) -> tuple[str | None, float | None, float | None] | None:
    try:
        url = f"http://ip-api.com/json/{ip}?fields=status,countryCode,lat,lon"
        with urllib.request.urlopen(url, timeout=3) as resp:
            data = json.loads(resp.read())
        if data.get("status") == "success":
            return (data.get("countryCode"), float(data["lat"]), float(data["lon"]))
    except Exception:
        pass
    return None
